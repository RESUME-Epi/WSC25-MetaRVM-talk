...
::: {#cdph .column}
![](figures/CDPH-Logo.png){width="44%"}

![](figures/cdph-heads.png){width="90%"}
:::
:::
...
::: column
![](figures/covid_icu_bed.png){width="110%"}
:::
...
::: {.column .callout}
![](figures/metarvm-screenshot.png){width="100%"}
:::
:::
...
![](figures/chisim-chicago.gif){width="90%"}
@macal2018chisim
:::
...
![](figures/chisim-mobility.gif){width="60%"}
:::

::: {.column}
#### ChiSIM
![](figures/colocation.gif){width="100%"}
Sample co-location network for randomly sampled individuals and all individuals within two degrees of separation.
@macal2018chisim
:::
...
ggplot(out_H$results, aes(x = date, y = value, color = age)) + 
  ...
  xlim(as.Date("2023-10-01"), as.Date("2024-07-25")) +
  ylim(0, 60) +
  ylab("new hospitalizations") + theme_bw()
...
yml_tmp <- data.table::copy(yml)
yml_tmp$simulation_config$start_date <- new_start_date
yml_tmp$simulation_config$length <- 90
yml_tmp$simulation_config$restore_from <- checkpoint_files[1]
yml_tmp$population_data$initialization <- NULL # ensure that we don't want to reinitialize the population from the initialization file
...
ggplot(out_H_all, aes(x = date, y = value, color = age)) + 
  geom_line() + 
  geom_vline(xintercept = as.Date("2024-01-27"), linetype = "dashed", linewidth = 1.3) + 
  geom_vline(xintercept = as.Date("2024-04-26"), linetype = "dashed", linewidth = 1.3) + 
  xlim(as.Date("2023-10-01"), as.Date("2024-07-25")) +
  ylim(0, 60) +
  ylab("new hospitalizations") + theme_bw()
...
# Restore and run
new_start_date <- "04/27/2024"
yml_tmp <- data.table::copy(yml)
yml_tmp$simulation_config$start_date <- new_start_date
yml_tmp$simulation_config$length <- 90
yml_tmp$simulation_config$restore_from <- checkpoint_files[2]
yml_tmp$population_data$initialization <- NULL # ensure that we don't want to reinitialize the population from the initialization file

temp_config_resume_2 <- tempfile(tmpdir = dirname(config_file),fileext = ".yaml")
yaml::write_yaml(yml_tmp, temp_config_resume_2)
config_resume_2 <- normalizePath(temp_config_resume_2)

out_resume_2 <- metaRVM(config_resume_2)

out_H <- out$subset_data(disease_states = "n_IsympH")
out_resume_H <- out_resume$subset_data(disease_states = "n_IsympH")
out_resume_H_2 <- out_resume_2$subset_data(disease_states = "n_IsympH")
out_H_all <- rbind(out_H$results, out_resume_H$results, out_resume_H_2$results)
